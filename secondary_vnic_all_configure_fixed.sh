...
oci_vcn_ensure_rt_tables() {
    # Ensure /etc/iproute2/rt_tables exists with standard entries.
    # Debian 13+ (iproute2 >= 6.x) no longer ships this file by default;
    # Debian 12, Ubuntu, RHEL/OL/CentOS all ship it via the iproute2 package.
    # This function is a no-op on systems that already have the file.
    if [ ! -f "$RTS_FILE" ]; then
        local rts_dir
        rts_dir=$(dirname "$RTS_FILE")
        if [ ! -d "$rts_dir" ]; then
            mkdir -p "$rts_dir"                 || oci_vcn_err "cannot create directory $rts_dir"
        fi
        printf '# Reserved values\n255\tlocal\n254\tmain\n253\tdefault\n0\tunspec\n\n# local\n'             > "$RTS_FILE"             || oci_vcn_err "cannot create route tables file $RTS_FILE"
        oci_vcn_info "created missing route tables file $RTS_FILE (Debian 13+ compat)"
    fi
}
